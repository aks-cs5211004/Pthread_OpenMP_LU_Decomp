...
void luDecomposition(double *a, double* l, double* u, int* pi, int n)
{
	for(int k=0 ; k<n ; k++)
    {
        cout<<k<<endl;
...
int main(int argc, char** argv)
{
	if (argc < 2)
	{
		cerr << "Usage: " << argv[0] << " <size of matrix>" << endl;
		return 1;
	}
	...
	double *lu1 = (double*)malloc(sizeof(double)*(n*n));
	matrixMult(l, u, lu1, n);
	// print2dArray(lu1, n);
	...
	luDecomposition(a, l, u, pi, n);
	auto end_seq = high_resolution_clock::now();
	duration<double> elapsed_seq = end_seq - start_seq;

	cout << "Sequential Time: " << elapsed_seq.count() << " seconds." << endl;
